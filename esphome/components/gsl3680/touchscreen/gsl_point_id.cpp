...
static void PointPredict(void) {
  ...
  for (i = 0; i < POINT_MAX; i++) {
    if (pp[1][i].all != 0) {
      if (global_state.other.interpolation != 0 && global_state.other.interpolation != INTE_INIT && pp[3][i].all &&
          pp[3][i].other.fill == 0) {
        if (pp[4][i].all && pp[5][i].all && pp[5][i].other.fill == 0)
          PointPredictD3(i);
        else
          PointPredictD2(i);
      } else if (global_state.other.interpolation || pp[2][i].all == 0 || pp[2][i].other.fill != 0 ||
                 pp[3][i].other.fill != 0 || pp[1][i].other.key != 0 || global_state.other.only) {
        PointPredictOne(i);
...
static unsigned int PointDistance(union gsl_POINT_TYPE *p1, union gsl_POINT_TYPE *p2) {
  ...
  if (id_flag.other.reso_y) {
    ...
    b = p2->dis.x;
    ret = (a - b) * (a - b);
    a = p1->dis.y * 64 * (int) screen_y_max / (int) screen_x_max * ((int) drv_num_nokey * 64) /
        ((int) sen_num_nokey * 64) / 64;
    b = p2->dis.y * 64 * (int) screen_y_max / (int) screen_x_max * ((int) drv_num_nokey * 64) /
        ((int) sen_num_nokey * 64) / 64;
    ret += (a - b) * (a - b);
  } else if (id_flag.other.reso_x) {
    a = p1->dis.x * 64 * (int) screen_x_max / (int) screen_y_max * ((int) sen_num_nokey * 64) /
        ((int) drv_num_nokey * 64) / 64;
    b = p2->dis.x * 64 * (int) screen_x_max / (int) screen_y_max * ((int) sen_num_nokey * 64) /
        ((int) drv_num_nokey * 64) / 64;
    ret = (a - b) * (a - b);
    a = p1->dis.y;
    b = p2->dis.y;
...
static void PointCross(void) {
  ...
  for (j = 0; j < POINT_MAX; j++) {
    for (i = j + 1; i < POINT_MAX; i++) {
      if (pp[0][i].all == 0 || pp[0][j].all == 0 || pp[1][i].all == 0 || pp[1][j].all == 0)
        continue;
...
static void PointDelay(void) {
  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    if (id_flag.other.first_avg && pp[0][i].all == 0 && pp[1][i].all != 0 && point_delay[i].other.able == 0 &&
...
static void PointMenu(void) {
  ...
  if (edge_start == 0)
    return;
  if (pp[0][0].all == 0 || pp[1][0].all == 0 || (pp[2][0].all != 0 && global_state.other.menu == 0) ||
      pp[3][0].all != 0) {
...
static unsigned int KeyMap(int *drv, int *sen) {
  ...
  for (i = 0; i < 8; i++) {
    if ((unsigned int) *drv >= (key_range[i].up_down >> 16) && (unsigned int) *drv <= (key_range[i].up_down & 0xffff) &&
...
static void PointEdge(void) {
  ...
  for (i = 0; i < 4; i++) {
    if (id_flag.other.screen_core)
      break;
    if (stretch->right[i].range > screen_y_max * 64 / 128 || stretch->down[i].range > screen_x_max * 64 / 128 ||
      ...
      if (dn < ds[0]) {
        for (j = 0; j <= len; j++) {
          if (j == len || dn == 0) {
            ...
            ps[0][i].other.x =
                (int) ps[1][i].other.x + (((int) pp[0][i].other.x - (int) ps[1][i].other.x) * dr + 64) / 128;
            break;
          }
        }
      }
      ...
      if (dn < ds[0]) {
        for (j = 0; j <= len; j++) {
          if (j == len || dn == 0) {
            ...
            dr = dc[j + 1] + ((dn - ds[j + 1]) * (dc[j] - dc[j + 1])) / (ds[j] - ds[j + 1]);
            ps[0][i].other.y =
                (int) ps[1][i].other.y + (((int) pp[0][i].other.y - (int) ps[1][i].other.y) * dr + 64) / 128;
            break;
          }
        }
      }
...
static void PointStretch(void) {
  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    if (ps[1][i].all == 0) {
      continue;
    } else if (id_flag.other.first_avg && (point_shake & (0x1 << i)) == 0 && pp[0][i].all &&
...
static void ResetMask(void) {
  ...
  else if (PointDistance((union gsl_POINT_TYPE *) (&reset_mask_max), pp[0]) >
...
static int DiagonalDistance(union gsl_POINT_TYPE *p, int type) {
  ...
  if (type == 0)
    ...
    square = ((int) sen_num_nokey * (int) (p->other.x) + (int) drv_num_nokey * (int) (p->other.y) -
...
int gsl_TouchNear(void) { return 0; }
...
unsigned int gsl_version_id(void) { return GSL_VERSION; }
...
static void GetFlag(void) {
  ...
  if (((point_num & 0x100) != 0) || ((point_num & 0x200) != 0 && global_state.other.reset == 1)) {
    gsl_id_reg_init(0);
  }
